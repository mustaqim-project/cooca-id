            ...
            <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
                <div class="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                    <div class="p-6 text-gray-900">
                        ...
                        <div class="mb-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
                            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                                ...
                                <div class="flex items-end">
                                    <button
                                        @click="statusFilter = 'all'; dateFrom = ''; dateTo = ''"
                        ...
                        <!-- Tabel -->
                        <div v-if="filteredWithdrawals(withdrawals).length === 0" class="text-center py-12">
                            <svg class="mx-auto h-12 w-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 14l6-6m-5.5.5a.5.5 0 010 1h5a.5.5 0 010 1h-5a.5.5 0 010-1m0 2a.5.5 0 010 1h5a.5.5 0 010 1h-5a.5.5 0 010-1m0 2a.5.5 0 010 1h5a.5.5 0 010 1h-5a.5.5 0 010-1" />
                            </svg>
                            <h3 class="mt-2 text-sm font-medium text-gray-900">Belum ada penarikan</h3>
                        ...
                        <div v-else class="overflow-x-auto">
                            <table class="min-w-full divide-y divide-gray-200">
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    <tr v-for="withdrawal in filteredWithdrawals(withdrawals)" :key="withdrawal.id" class="hover:bg-gray-50">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                            Rp {{ withdrawal.fee.toLocaleString('id-ID') }}
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-indigo-600">
                                            Rp {{ withdrawal.net_amount.toLocaleString('id-ID') }}
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 capitalize">
                                            {{ withdrawal.method === 'bank' ? 'Bank' : 'E-Wallet' }}
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <span :class="['px-2 py-1 text-xs rounded-full', statusColors[withdrawal.status]]">
                                                {{ statusLabels[withdrawal.status] }}
                                            </span>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <Link
                                                :href="route('affiliator.withdrawals.show', withdrawal.id)"
