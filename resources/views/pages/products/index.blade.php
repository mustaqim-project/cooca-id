                    ...
                    @foreach ($products as $product)
                        <div class="col-lg-4 col-md-6 reveal">
                            <div class="card card-3d product-card card-hover-glow">
                                ...
                                <p class="card-desc">
                                    {{ Str::limit($product->description ?? $product->short_description, 100) }}</p>
                                <div class="card-actions">
                                    <a href="{{ route('products.show', $product->slug) }}"
                                        class="btn btn-outline btn-sm">{{ __('Details') }} <i
                                            class="bi bi-arrow-right"></i></a>
                                    @if ($product->subscriptionPlans && $product->subscriptionPlans->count())
                                        <span
                                            style="font-size:0.85rem;color:var(--accent);display:flex;align-items:center;">
                                            {{ __('From') }} {{ setting('currency.symbol', 'Rp') }}
                                            {{ number_format($product->subscriptionPlans->min('price'), 0, ',', '.') }}
                                        </span>
                                    @endif
                                </div>
                            </div>
