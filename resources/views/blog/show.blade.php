...
@section('title', $post->title . ' - Blog Cooca.id')
    ...
    <div class="relative h-64 md:h-96 bg-gray-200">
        ...
        <div class="w-full h-full bg-gradient-to-br from-blue-400 to-purple-600 flex items-center justify-center">
            <div class="text-center text-white">
                <h1 class="text-3xl md:text-5xl font-bold mb-2">{{ substr($post->title, 0, 2) }}</h1>
                <p class="text-lg opacity-80">Blog Cooca.id</p>
            </div>
        </div>
        @endif
        ...
        <div class="absolute bottom-0 left-0 right-0 p-6 md:p-12">
            <div class="max-w-4xl mx-auto">
                <div class="flex items-center gap-2 mb-4">
                    ...
                    @if($post->is_featured)
                    <span class="px-3 py-1 text-sm font-medium bg-yellow-500 text-white rounded-full">
                        ⭐ Featured
                    </span>
                    @endif
                </div>
                ...
                <div class="flex items-center gap-4 text-white/90">
                    <div class="flex items-center gap-2">
                        ...
                        <div>
                            <p class="font-medium">{{ $post->author?->name ?? 'Admin' }}</p>
                            <p class="text-sm text-white/70">Author</p>
                        </div>
                    </div>
                    <div class="h-8 w-px bg-white/30"></div>
                    <div>
                        <p class="font-medium">{{ $post->published_at ? $post->published_at->format('d F Y') : 'Draft' }}</p>
                        <p class="text-sm text-white/70">Published</p>
                    </div>
                    <div class="h-8 w-px bg-white/30"></div>
                    <div>
                        <p class="font-medium">👁 {{ $post->view_count ?? 0 }}</p>
                        <p class="text-sm text-white/70">Views</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div class="mt-12 pt-8 border-t border-gray-200">
            ...
            <div class="flex flex-wrap gap-2">
                ...
                <a href="{{ route('blog.index', ['search' => $tag]) }}" class="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition-colors text-sm">
                    #{{ $tag }}
                </a>
                @endforeach
            </div>
        ...
        <div class="mt-8 pt-8 border-t border-gray-200">
            ...
            <div class="flex gap-3">
                ...
                <button onclick="navigator.clipboard.writeText('{{ route('blog.show', $post->slug) }}'); alert('Link copied!');" 
            ...
            <article class="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300">
                <div class="aspect-w-16 aspect-h-9 bg-gray-200">
                    ...
                    <div class="w-full h-40 bg-gradient-to-br from-blue-400 to-purple-500 flex items-center justify-center">
                        <span class="text-2xl font-bold text-white">{{ substr($related->title, 0, 2) }}</span>
                    </div>
                    @endif
                </div>
                ...
                <div class="p-4">
                    ...
                    <div class="flex items-center justify-between text-xs text-gray-500">
                        <span>{{ $related->published_at ? $related->published_at->format('d M Y') : 'Draft' }}</span>
...
<!-- CTA Section -->
<section class="py-12 bg-gradient-to-br from-blue-600 to-purple-700">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        <h2 class="text-3xl font-bold text-white mb-4">Butuh Solusi ERP untuk Bisnis Anda?</h2>
        <p class="text-blue-100 text-lg mb-8">
            Konsultasikan kebutuhan bisnis Anda dengan tim ahli kami dan dapatkan solusi ERP yang tepat.
        </p>
        <div class="flex flex-col sm:flex-row gap-4 justify-center">
            <a href="{{ route('pricing') }}" class="px-8 py-3 bg-white text-blue-600 rounded-lg hover:bg-blue-50 transition-colors font-semibold">
                Lihat Pricing
            </a>
            <a href="{{ route('contact') }}" class="px-8 py-3 bg-transparent border-2 border-white text-white rounded-lg hover:bg-white/10 transition-colors font-semibold">
                Hubungi Kami
            </a>
        </div>
    </div>
