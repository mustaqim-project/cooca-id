    ...
    <div class="container">
        <div class="row g-4">
            <div class="col-lg-4 col-md-6">
                    ...
                    @if (setting('site.logo_light') || setting('site.logo_dark'))
                        @if (setting('site.logo_light'))
                            <img src="{{ asset(setting('site.logo_light')) }}" alt="{{ setting('site.name', 'COOCA') }}"
                                class="nav-logo-light" style="height:28px;object-fit:contain;" />
                        @endif
                        @if (setting('site.logo_dark'))
                            <img src="{{ asset(setting('site.logo_dark')) }}" alt="{{ setting('site.name', 'COOCA') }}"
                                class="nav-logo-dark" style="height:28px;object-fit:contain; display:none;" />
                        @endif
                    @elseif(setting('site.logo'))
                        <img src="{{ asset(setting('site.logo')) }}" alt="{{ setting('site.name', 'COOCA') }}"
                            class="nav-logo-default" style="height:28px;object-fit:contain;" />
                    @else
                ...
                <p class="footer-desc">
                    {{ __(setting('footer.description', 'The business system that works like an asset. Lifetime license, modular ERP, and complete digital infrastructure for serious businesses.')) }}
                </p>
                <div class="footer-socials">
                    @if (setting('social.twitter'))
                        <a href="{{ setting('social.twitter', '#') }}" aria-label="Twitter"><i
                                class="bi bi-twitter-x"></i></a>
                    @endif
                    @if (setting('social.linkedin'))
                        <a href="{{ setting('social.linkedin', '#') }}" aria-label="LinkedIn"><i
                                class="bi bi-linkedin"></i></a>
                    @endif
                    @if (setting('social.github'))
                        <a href="{{ setting('social.github', '#') }}" aria-label="GitHub"><i
                                class="bi bi-github"></i></a>
                    @endif
                    @if (setting('social.youtube'))
                        <a href="{{ setting('social.youtube', '#') }}" aria-label="YouTube"><i
                                class="bi bi-youtube"></i></a>
                    @endif
                    @if (setting('social.instagram'))
                        <a href="{{ setting('social.instagram', '#') }}" aria-label="Instagram"><i
                ...
                <div class="mt-3">
                    ...
                    <form action="{{ route('newsletter.subscribe') }}" method="POST" class="d-flex gap-1 mt-1">
                        ...
                        <input type="email" name="email" class="form-control form-control-sm"
                            ...
                            style="background:var(--card);border-color:var(--border);color:var(--text);border-radius:8px;font-size:0.8rem;"
                            required>
                        <button type="submit" class="btn btn-primary btn-sm"
                            style="white-space:nowrap;border-radius:8px;padding:6px 10px;font-size:0.75rem;">
        ...
        <div class="footer-bottom">
            <p>&copy; {{ date('Y') }} {{ setting('site.name', 'COOCA') }}.
                {{ __(setting('footer.copyright_text', 'All rights reserved.')) }}</p>
