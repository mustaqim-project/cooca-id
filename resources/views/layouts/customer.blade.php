            ...
            @php
                $navigation = [
                    ...
                    [
                        ...
                        'icon' => 'bi-box',
                        'route_name' => 'customer.products.*',
                    ],
                    [
                        'name' => 'Subscripsi',
                        'href' => route('customer.subscriptions.index'),
                        'icon' => 'bi-arrow-repeat',
                        'route_name' => 'customer.subscriptions.*',
                    ],
                    [
                        'name' => 'Lisensi',
                        'href' => route('customer.licenses.index'),
                        'icon' => 'bi-key',
                        'route_name' => 'customer.licenses.*',
                    ],
                    [
                        'name' => 'Invoice',
                        'href' => route('customer.invoices.index'),
                        'icon' => 'bi-file-text',
                        'route_name' => 'customer.invoices.*',
                    ],
                ];
            @endphp
                    ...
                    <button @click="open = !open" @click.away="open = false"
                        ...
                        <div class="hidden md:block text-left">
                            <p class="text-sm font-medium text-surface-700 dark:text-surface-200">
                                {{ auth('customer')->user()->name ?? 'Customer' }}</p>
                            <p class="text-xs text-surface-500 dark:text-surface-400">
                                {{ auth('customer')->user()->business_name ?? '-' }}</p>
                        </div>
