...
<body class="bg-surface-50 text-surface-900 dark:bg-surface-900 dark:text-surface-100 font-sans antialiased flex h-screen overflow-hidden transition-colors duration-200">
    ...
    <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'" class="fixed inset-y-0 left-0 z-50 w-64 corporate-sidebar transition-transform duration-300 -translate-x-full lg:translate-x-0 lg:static lg:inset-auto flex flex-col h-screen shrink-0">
        ...
        <div class="h-16 flex items-center justify-center border-b border-surface-200 dark:border-surface-700 shrink-0">
            <a href="{{ route('affiliator.dashboard') }}" class="text-xl font-bold tracking-tight text-surface-900 dark:text-white">
        ...
        <nav class="flex-1 overflow-y-auto p-4 space-y-2">
            @php
                $navigation = [
                    ['name' => 'Dashboard', 'href' => route('affiliator.dashboard'), 'icon' => 'bi-house-door', 'route_name' => 'affiliator.dashboard'],
                    ['name' => 'Referrals', 'href' => route('affiliator.referrals.index'), 'icon' => 'bi-person-plus', 'route_name' => 'affiliator.referrals.*'],
                    ['name' => 'Commissions', 'href' => route('affiliator.commissions.index'), 'icon' => 'bi-currency-dollar', 'route_name' => 'affiliator.commissions.*'],
                    ['name' => 'Downlines', 'href' => route('affiliator.downlines.index'), 'icon' => 'bi-people', 'route_name' => 'affiliator.downlines.*'],
                    ['name' => 'Withdrawals', 'href' => route('affiliator.withdrawals.index'), 'icon' => 'bi-cash-stack', 'route_name' => 'affiliator.withdrawals.*'],
                ];
            @endphp
                ...
                <div class="relative" x-data="{ open: false }">
                    <button @click="open = !open" @click.away="open = false" class="flex items-center space-x-3 focus:outline-none">
                        ...
                        <div class="hidden md:block text-left">
                            <p class="text-sm font-medium text-surface-700 dark:text-surface-200">{{ auth('affiliator')->user()->name ?? 'Affiliator' }}</p>
                            <p class="text-xs font-semibold text-emerald-600 dark:text-emerald-400">Rp {{ number_format(auth('affiliator')->user()->balance ?? 0, 0, ',', '.') }}</p>
                        </div>
                    ...
                    <div x-show="open" x-transition class="absolute right-0 mt-2 w-48 bg-white dark:bg-surface-800 rounded-md shadow-lg py-1 ring-1 ring-black ring-opacity-5 focus:outline-none" style="display: none;">
                        <a href="{{ route('affiliator.profile.edit') }}" class="block px-4 py-2 text-sm text-surface-700 dark:text-surface-300 hover:bg-surface-100 dark:hover:bg-surface-700">Profile</a>
