                    ...
                    <form id="subscribeForm" method="POST" action="{{ route('customer.subscriptions.store') }}">
                        ...
                        @php
                            $waCustomDomainMsg = urlencode(
                                'Halo COOCA.ID, saya ingin request setup custom domain untuk langganan baru saya.',
                            );
                        @endphp
                        ...
                        <div id="subdomain_container" class="form-group"
                            ...
                            <div class="form-hint" id="subdomain_hint">Akses ERP Anda di <code>namabisnis.cooca.id</code>
                            </div>
                        </div>

                        <div id="custom_domain_container" class="form-group"
                            style="{{ request('domain_type') === 'custom' ? '' : 'display:none;' }}">
                            ...
                            <input type="text"
                                ...
                                id="custom_domain_input" class="form-input" placeholder="contoh: erp.bisnisanda.com"
                                autocomplete="off"
                                value="{{ request('domain_type') === 'custom' ? old('domain', $preDomain) : '' }}"
                                {{ request('domain_type') === 'custom' ? 'required' : '' }}>

                            <div
                                style="margin-top: 12px; padding: 12px; border-radius: 8px; background: color-mix(in srgb, var(--primary) 5%, transparent); border: 1px dashed var(--primary);">
                                <div class="text-xs text-muted mb-2">
                                    <i class="fa-solid fa-circle-info" style="color:var(--primary);"></i>
                                    Gunakan domain Anda sendiri. Setelah pembayaran berhasil, silakan hubungi CS kami.
                                    <strong>Catatan:</strong> Jika Anda belum memiliki domain sendiri, akan dikenakan biaya
                                    tambahan untuk pembelian domain dan Anda akan dihubungi oleh admin untuk proses lebih
                                    lanjut.
                                </div>
                                <a href="https://wa.me/{{ $waNumber }}?text={{ $waCustomDomainMsg }}"
                                    target="_blank" class="btn btn-outline btn-sm w-full"
                                    style="justify-content:center; border-color:#25D366; color:#25D366;">
