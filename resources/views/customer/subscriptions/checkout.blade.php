...
@section('content')
@php
    $prod = $subscription->subscriptionPlan?->product
         ?? $subscription->license?->product;
@endphp
...
@if($pendingTransaction)
    <div class="alert alert-warning mb-4">
        ...
        <strong>Tagihan Pending Ditemukan:</strong> Terdapat transaksi renewal yang belum dibayar (<code>{{ $pendingTransaction->invoice_number }}</code>).
        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="flex items-center gap-4">
                    ...
                    @else
                        <div class="product-logo-placeholder" style="width:64px;height:64px;font-size:26px;border-radius:12px;flex-shrink:0;">
            ...
            <div class="card-body" style="display:flex;flex-direction:column;gap:8px;">
                @foreach(collect($plan->product->features)->take(6) as $feature)
                    <div class="flex items-center gap-2 text-sm">
                        <i class="{{ is_array($feature) ? ($feature['icon'] ?? 'fa-solid fa-check-circle') : 'fa-solid fa-check-circle' }}" style="color:var(--success);font-size:13px;flex-shrink:0;"></i>
        ...
        <div class="card">
            ...
            <div class="card-body" style="display:flex;flex-direction:column;gap:12px;">
                ...
                <div class="stats-row">
                    ...
                    <span class="font-bold" style="color:var(--success);">- Rp {{ number_format($discountAmount, 0, ',', '.') }}</span>
                </div>
                @endif
                ...
                <div style="display:flex;flex-direction:column;gap:8px;">
                    ...
                    <div style="display:flex;gap:8px;">
                        <input type="text" id="voucher-code" class="form-input" placeholder="Masukkan kode promo..." style="flex:1;">
                        <button type="button" id="btn-apply-voucher" class="btn btn-primary" style="padding:8px 16px;">Gunakan</button>
                    </div>
                ...
                <div id="voucher-discount-row" class="stats-row" style="display:none;">
                    <span class="text-sm" style="color:var(--success);">Voucher (<span id="applied-voucher-code"></span>)</span>
                    <span class="font-bold" style="color:var(--success);">- Rp <span id="voucher-discount-amount">0</span></span>
                </div>
            ...
            <div class="card-body" style="display:flex;flex-direction:column;gap:12px;">
                ...
                @if(isset($snapToken) && $snapToken)
                    ...
                    <div style="text-align:center;padding:16px 0 8px;">
                        <i class="fa-solid fa-circle-check" style="color:var(--success);font-size:32px;margin-bottom:8px;display:block;"></i>
                        <div class="font-bold text-sm">Transaksi dibuat!</div>
                        <div class="text-xs text-muted mt-1">Klik tombol di bawah untuk membuka halaman pembayaran.</div>
                    </div>
                ...
                @else
                    ...
                    <div style="display:flex;flex-direction:column;gap:8px;">
                        <div class="flex items-center gap-2 p-3" style="border:1px solid var(--border);border-radius:var(--radius);background:var(--bg);">
                            <i class="fa-solid fa-building-columns" style="color:var(--primary);font-size:18px;flex-shrink:0;"></i>
                            <div>
                    ...
                    <form method="POST" action="{{ route('customer.subscriptions.checkout.process', $subscription->id) }}">
                        ...
                        <button type="submit" class="btn btn-primary w-full" id="btn-pay-submit" style="justify-content:center;font-size:15px;padding:14px;">
                            <i class="fa-solid fa-lock"></i>
                            Lanjutkan Pembayaran — Rp <span id="btn-net-amount">{{ number_format($netAmount, 0, ',', '.') }}</span>
                        </button>
                    </form>
                @endif

                <div class="text-xs text-muted" style="text-align:center;">
                    Dengan melanjutkan, Anda menyetujui <span style="color:var(--primary);">Syarat & Ketentuan</span> layanan kami.
                </div>
            </div>
        </div>

        {{-- Info box --}}
        <div style="border:1px solid var(--border);border-radius:var(--radius);padding:14px;display:flex;flex-direction:column;gap:8px;">
            <div class="text-sm font-bold"><i class="fa-solid fa-circle-info" style="color:var(--primary);margin-right:6px;"></i>Info Pembayaran</div>
