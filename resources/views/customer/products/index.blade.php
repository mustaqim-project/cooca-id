...
@section('content')
    @php
        ...
        $totalActive =
            $activeSubs->where('status', 'active')->count() + $activeTrials->where('status', 'active_trial')->count();
    @endphp
    ...
    {{-- ── TABS ── --}}
    <div style="display:flex;gap:4px;border-bottom:2px solid var(--border);margin-bottom:24px;overflow-x:auto;"
        id="serviceTabs">
        ...
        <button class="tab-btn" onclick="switchTab('trials')" id="tab-trials">
            ...
            @if ($activeTrials->count() > 0)
                <span class="badge badge-warning"
                    style="margin-left:4px;font-size:10px;">{{ $activeTrials->count() }}</span>
            @endif
        </button>
    </div>
        ...
        <div class="grid-3">
            @forelse($products ?? [] as $product)
                @php
                    ...
                    $hasActive =
                        $activeSubs
                            ->where('subscriptionPlan.product.id', $product->id)
                    ...
                    <div class="card-body" style="flex:1;display:flex;flex-direction:column;gap:12px;">
                        <div class="flex items-start justify-between gap-3">
                            <div class="flex items-center gap-3">
                                ...
                                @elseif($product->icon)
                                    <div
                                        style="width:52px;height:52px;border-radius:12px;background:linear-gradient(135deg,var(--primary),#7C3AED);display:flex;align-items:center;justify-content:center;font-size:22px;flex-shrink:0;">
                                        {{ $product->icon }}</div>
                                @else
                                    <div class="product-logo-placeholder"
                                        style="width:52px;height:52px;font-size:20px;border-radius:12px;flex-shrink:0;">
                        ...
                        <p class="text-xs text-muted"
                            style="line-height:1.6;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;flex:1;">
                        ...
                        <div style="display:flex;align-items:center;justify-content:space-between;">
                            <div>
                                ...
                                @elseif($origPrice > 0 || $finalPrice > 0)
                                    ...
                                    @if ($discount > 0)
                                        <div class="text-xs text-muted" style="text-decoration:line-through;">Rp
                                            {{ number_format($origPrice, 0, ',', '.') }}</div>
                                        <div class="font-bold text-base" style="color:var(--primary);">Rp
                                            {{ number_format($finalPrice, 0, ',', '.') }}<span
                                                class="text-xs text-muted font-normal">/bln</span></div>
                                    @else
                                        <div class="font-bold text-base" style="color:var(--primary);">Rp
                                            {{ number_format($origPrice, 0, ',', '.') }}<span
                                                class="text-xs text-muted font-normal">/bln</span></div>
                                    @endif
                                @else
                            ...
                            <div class="flex gap-1">
                                ...
                                @if (!$isCustom)
                                    <span class="badge badge-purple" style="font-size:9px;">Trial</span>
                                @endif
                            </div>
                        </div>
                        ...
                        <div style="display:flex;gap:8px;margin-top:4px;">
                            @if ($isCustom)
                                ...
                                <a href="https://wa.me/{{ $waNumber }}?text={{ urlencode('Halo, saya ingin info lebih lanjut tentang ' . $product->name) }}"
                                    target="_blank" class="btn btn-primary btn-sm w-full" style="justify-content:center;">
                                ...
                                @if ($hasActive)
                                    <div style="display:flex;gap:6px;width:100%;">
                                        ...
                                        <a href="{{ route('customer.products.show', $product->slug ?? $product->id) }}"
                                            ...
                                            title="Order instance baru dengan domain berbeda">
            ...
            @empty
                <div class="card" style="grid-column:1/-1;">
                    <div class="empty-state">
                        ...
                        <div class="empty-state-text">Hubungi tim kami untuk solusi enterprise sesuai kebutuhan bisnis
                            Anda.</div>
                    ...
                    <div class="card card-hover">
                        <div class="card-body">
                            <div class="flex items-start justify-between gap-3 mb-3">
                                <div class="flex items-center gap-3">
                                    @if ($prod?->thumbnail)
                                        <img src="{{ $prod->thumbnail_url }}" alt="{{ $prod->name }}"
                                            style="width:48px;height:48px;border-radius:10px;object-fit:contain;border:1px solid var(--border);background:var(--bg);padding:4px;flex-shrink:0;">
                                    @else
                                        <div class="product-logo-placeholder"
                                            style="width:48px;height:48px;font-size:18px;border-radius:10px;flex-shrink:0;">
                                            {{ strtoupper(substr($prod?->name ?? 'S', 0, 1)) }}
                                        </div>
                                    @endif
                                    <div>
                                        <div class="font-bold text-base">{{ $prod?->name ?? 'Subscription' }}</div>
                                        <div class="text-xs text-muted">{{ $plan?->name }} · {{ $plan?->billing_cycle }}
                                        </div>
                                    </div>
                                </div>
                                <div style="flex-shrink:0;">
                                    ...
                                    @elseif($sub->status === 'trial')
                                        <span class="badge badge-muted">Pending Payment</span>
                                    @else
                                        <span class="badge badge-muted">{{ ucfirst($sub->status) }}</span>
                                    @endif
                                </div>
                            </div>
                            ...
                            <div style="display:flex;flex-direction:column;gap:6px;margin-bottom:14px;">
                                @if ($daysLeft !== null)
                                    <div class="stats-row" style="margin:0;">
                                        ...
                                        <span
                                            class="text-xs font-bold {{ $soonExpire || $isExpired ? 'text-danger' : '' }}">
                                ...
                                @if ($lic?->domain)
                                    <div class="stats-row" style="margin:0;">
                                        ...
                                        <a href="https://{{ $lic->domain }}" target="_blank"
                                            class="text-xs text-primary font-semibold">
                                            {{ $lic->domain }} <i class="fa-solid fa-arrow-up-right-from-square"
                            ...
                            @if ($plan?->duration_months && $plan->duration_months < 999 && $daysLeft !== null && $sub->started_at)
                                ...
                                @endphp
                                <div
                                    style="background:var(--border);border-radius:4px;height:5px;margin-bottom:12px;overflow:hidden;">
                                    <div
                                        style="width:{{ $pct }}%;background:{{ $barColor }};height:100%;transition:width .4s;border-radius:4px;">
                                    </div>
                                </div>
                            @endif
                            ...
                            <div class="flex gap-2">
                                ...
                                @endif
                                @if ($sub->status === 'trial')
                                    <a href="{{ route('customer.subscriptions.checkout', $sub->id) }}"
        ...
        @if ($activeTrials->count() === 0)
            <div class="card">
                <div class="empty-state">
                    ...
                    <div class="empty-state-text">Coba modul COOCA.ID secara gratis selama 14 hari — tanpa kartu kredit.
                    </div>
            ...
            <div class="grid-2">
                @foreach ($activeTrials as $trial)
                    @php
                        ...
                        $trialDays = $trial->expires_at
                            ? max(0, (int) now()->diffInDays($trial->expires_at, false))
                            : null;
                    ...
                    <div class="card card-hover">
                        <div class="card-body">
                            <div class="flex items-start justify-between gap-3 mb-3">
                                <div class="flex items-center gap-3">
                                    <div class="product-logo-placeholder"
                                        style="width:48px;height:48px;font-size:18px;border-radius:10px;background:linear-gradient(135deg,#8B5CF6,#6366F1);color:#fff;flex-shrink:0;">
                                ...
                                @if ($trialExp)
                                    <span class="badge badge-danger" style="flex-shrink:0;">Expired</span>
                                @elseif($trial->status === 'active')
                                    <span class="badge badge-success" style="flex-shrink:0;">Active</span>
                                @elseif($trial->status === 'pending')
                                    <span class="badge badge-warning" style="flex-shrink:0;">Pending</span>
                                @else
                                    <span class="badge badge-muted"
                                        style="flex-shrink:0;">{{ ucfirst($trial->status) }}</span>
                                @endif
                            </div>
                            ...
                            @if ($trial->domain)
                                <div class="stats-row mb-3" style="margin:0 0 12px;">
                                    <span class="text-xs text-muted">Domain</span>
                                    <a href="https://{{ $trial->domain }}" target="_blank"
                                        class="text-xs text-primary font-semibold">
                                        {{ $trial->domain }} <i class="fa-solid fa-arrow-up-right-from-square"
                                            style="font-size:9px;"></i>
                                    </a>
                                </div>
                            @endif
                            ...
                            @if ($trialDays !== null)
                                <div class="stats-row mb-3" style="margin:0 0 12px;">
                                    ...
                                    <span
                                        class="text-xs font-bold {{ $trialDays <= 3 ? 'text-danger' : '' }}">{{ $trialDays }}
                                        hari</span>
                                </div>
                            @endif
                            ...
                            <div class="flex gap-2">
                                ...
                                @endif
                                <a href="{{ route('customer.trials.show', $trial->id) }}" class="btn btn-outline btn-sm"
    ...
    <script>
        ...
        function confirmCancel(subId, prodName) {
            if (confirm('Apakah Anda yakin ingin membatalkan langganan "' + prodName +
                    '" yang belum dibayar? Semua data lisensi terkait akan dihapus.')) {
