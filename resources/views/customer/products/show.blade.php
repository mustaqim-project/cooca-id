            ...
            <h1 class="page-title" style="font-size:22px;">
                @if ($product->thumbnail)
                    <img src="{{ $product->thumbnail_url }}"
                        style="width:36px;height:36px;border-radius:8px;object-fit:cover;vertical-align:middle;margin-right:10px;border:1px solid var(--border);">
                @endif
                {{ $product->name }}
            </h1>
        ...
        <div class="alert alert-success mb-4"
            ...
            <div>
                <div style="font-size:14px;font-weight:700;">
                    <i class="fa-solid fa-circle-check"></i> Langganan Aktif: {{ $activeSub->subscriptionPlan?->name }}
                    (<code>{{ $activeSub->license?->domain }}</code>)
                </div>
                <div class="text-xs mt-1" style="color:var(--text-muted);">
                    @if ($activeSub->expires_at)
                        Masa aktif s/d <strong>{{ $activeSub->expires_at->format('d M Y') }}</strong> ·
                    @endif
                    💡 Ingin memesan <strong>instance / cabang tambahan</strong>? Pilih paket dan tentukan <strong>nama
                        domain baru</strong> pada formulir di bawah.
                </div>
            </div>
        ...
        <div class="alert alert-warning mb-4"
            ...
            <div>
                ...
                @if ($activeTrial->expires_at)
                    · Berakhir {{ \Carbon\Carbon::parse($activeTrial->expires_at)->format('d M Y') }}
                @endif
            </div>
        ...
        <div style="display:flex;flex-direction:column;gap:20px;">
            ...
            @if ($isCustom)
                ...
                <div class="card" style="border:2px solid #25D366;">
                    <div class="card-header"
                        style="background:linear-gradient(135deg,#25D366,#128C7E);border-radius:var(--radius-lg) var(--radius-lg) 0 0;">
                        <div class="card-title" style="color:#fff;"><i class="fa-brands fa-whatsapp"
                                style="margin-right:6px;"></i> Produk Custom — Hubungi Sales</div>
                    </div>
                    <div class="card-body">
                        <p class="text-sm text-muted mb-4" style="line-height:1.7;">
                            Produk <strong>{{ $product->name }}</strong> adalah solusi custom yang dikerjakan sesuai
                        ...
                        <a href="https://wa.me/{{ $waNumber }}?text={{ $waMsg }}" target="_blank"
                            ...
                            style="justify-content:center;font-size:15px;padding:14px;background:#25D366;border-color:#25D366;">
                            <i class="fa-brands fa-whatsapp" style="font-size:18px;"></i>
                            Chat WhatsApp — Konsultasi Gratis
                        </a>
                        @if ($product->demo_url)
                            <a href="{{ $product->demo_url }}" target="_blank" class="btn btn-outline w-full mt-2"
                            ...
                            <form method="GET" action="{{ route('customer.subscriptions.create') }}" id="subscribeForm">
                                ...
                                <div style="display:flex;flex-direction:column;gap:10px;margin-bottom:16px;">
                                    @foreach ($plans as $idx => $plan)
                                        <label class="plan-card-select {{ $idx === 0 ? 'selected' : '' }}"
                                            for="plan_{{ $plan->id }}">
                                            <input type="radio" name="plan" id="plan_{{ $plan->id }}"
                                                value="{{ $plan->id }}" {{ $idx === 0 ? 'checked' : '' }}
                                            ...
                                            <div class="flex items-center justify-between">
                                                ...
                                                <div class="text-right">
                                                    @php
                                                        ...
                                                        $finalPrice =
                                                            $discount > 0
                                                                ? $origPrice * (1 - $discount / 100)
                                                                : $origPrice;
                                                    @endphp
                                                    @if ($discount > 0)
                                                        <div class="text-xs text-muted"
                                                            style="text-decoration: line-through;">Rp
                                                            {{ number_format($origPrice, 0, ',', '.') }}</div>
                                                        <div class="font-bold" style="color:var(--primary);">Rp
                                                            {{ number_format($finalPrice, 0, ',', '.') }}</div>
                                                        <div class="text-xs"
                                                            style="color:var(--success); font-weight:600;">Hemat
                                                            {{ number_format($discount, $discount == floor($discount) ? 0 : 2) }}%
                                                        </div>
                                                    @else
                                                        <div class="font-bold" style="color:var(--primary);">Rp
                                                            {{ number_format($origPrice, 0, ',', '.') }}</div>
                                ...
                                @php
                                    ...
                                    $waCustomDomainMsg = urlencode(
                                        "Halo COOCA.ID, saya ingin request setup custom domain untuk langganan {$product->name} saya.",
                                    );
                                @endphp

                                {{-- Step 3: Domain Selection with Hostinger Integration --}}
                                <div class="form-group mb-3">
                                    <label class="form-label font-bold text-sm"
                                    ...
                                    <div class="flex gap-4" style="flex-wrap: wrap;">
                                        <label class="flex items-center gap-2 cursor-pointer p-2 rounded"
                                            ...
                                            <span class="font-medium text-sm">Subdomain COOCA.ID <span
                                                    class="badge badge-success"
                                                    style="font-size:10px; margin-left:4px;">Gratis</span></span>
                                        </label>
                                        <label class="flex items-center gap-2 cursor-pointer p-2 rounded"
                                            style="border:1px solid var(--border); background:var(--card); color:var(--text);">
                                ...
                                <div id="subdomain_container" class="form-group mb-4">
                                    <label class="form-label">Subdomain Instance <span
                                            style="color:var(--danger);">*</span></label>
                                    <div class="flex items-center gap-2">
                                        <input type="text" name="domain" id="subdomain_input" class="form-input"
                                            placeholder="namabisnisanda" pattern="[a-zA-Z0-9\-]+"
                                            title="Huruf kecil, angka, dan tanda hubung" autocomplete="off" required
                                    ...
                                    @if ($trialSubdomain)
                                        <div class="form-hint" style="color:var(--success); margin-top: 6px;">
                                            <i class="fa-solid fa-check-circle"></i> Terisi otomatis dari subdomain trial
                                            Anda sebelumnya.
                                        </div>
                                    @else
                                        <div class="form-hint" id="subdomain_hint">Instance Anda akan dapat diakses di
                                            <code>namabisnis.cooca.id</code></div>
                                    @endif
                                </div>
                                ...
                                <div id="custom_domain_container" class="form-group mb-4" style="display:none;">
                                    ...
                                    <div class="flex gap-2 mb-3 p-1 rounded-lg"
                                        ...
                                        <button type="button" id="tabBtnBuy" onclick="switchCustomDomainTab('buy')"
                                            ...
                                            <i class="fa-solid fa-cart-shopping"></i> Cari & Beli Domain Baru <span
                                                class="badge"
                                                style="background:rgba(255,255,255,0.2); font-size:9px; margin-left:4px;">Hostinger
                                                API</span>
                                        </button>
                                        <button type="button" id="tabBtnConnect"
                                            onclick="switchCustomDomainTab('connect')"
                                    ...
                                    <input type="hidden" name="custom_domain_action" id="custom_domain_action"
                                        value="buy">
                                    <input type="hidden" name="domain_price" id="selected_domain_price" value="0">
                                    <input type="hidden" name="custom_domain_ignore" id="final_custom_domain"
                                        value="">
                                    ...
                                    <div id="tab_buy_domain" style="display:block;">
                                        <div class="p-3 rounded-lg mb-3"
                                            ...
                                            <label class="form-label text-xs font-semibold text-muted mb-2">Cari
                                                Ketersediaan & Harga Domain (Live Kurs)</label>
                                            <div class="flex gap-2">
                                                <div style="position:relative; flex:1;">
                                                    <input type="text" id="hostinger_search_input"
                                                ...
                                                <button type="button" id="btnCheckHostinger"
                                                    onclick="searchHostingerDomain()" class="btn btn-primary btn-sm px-4"
                                            ...
                                            <div class="text-xs text-muted mt-2"
                                                ...
                                                <span class="badge cursor-pointer"
                                                    onclick="setSearchQuery('bisnisku')">.com</span>
                                                <span class="badge cursor-pointer"
                                                    onclick="setSearchQuery('bisnisku')">.id</span>
                                                <span class="badge cursor-pointer"
                                                    onclick="setSearchQuery('bisnisku')">.co.id</span>
                                                <span class="badge cursor-pointer"
                                                    onclick="setSearchQuery('bisnisku')">.shop</span>
                                                <span class="badge cursor-pointer"
                                                    onclick="setSearchQuery('bisnisku')">.online</span>
                                            </div>
                                        </div>

                                        {{-- Search Results / State --}}
                                        <div id="hostinger_loading"
                                            style="display:none; text-align:center; padding:20px; color:var(--primary);">
                                            <i class="fa-solid fa-circle-notch fa-spin fa-2x mb-2"></i>
                                            <div class="text-xs font-semibold">Mengecek ketersediaan & harga domain di
                                                Hostinger...</div>
                                        </div>

                                        <div id="hostinger_results_container" style="display:none;">
                                            <div class="text-xs font-bold mb-2 text-muted uppercase tracking-wider"
                                                ...
                                                <div style="display:flex; align-items:center; gap:8px;">
                                                    <span id="hostinger_rate_badge" class="badge"
                                                        style="background:var(--bg-secondary); border:1px solid var(--border); font-size:10px; color:var(--text-muted); font-weight:normal;">
                                                        <i class="fa-solid fa-coins" style="color:#F59E0B;"></i> Kurs Live
                                                    </span>
                                                    <span class="text-primary font-normal text-xs"><i
                                                            class="fa-solid fa-bolt"></i> Realtime Catalog</span>
                                                </div>
                                            </div>
                                            <div id="hostinger_domain_list"
                                            ...
                                            <div id="selected_domain_banner" class="mt-3 p-3 rounded-lg"
                                                ...
                                                <div
                                                    ...
                                                    <div>
                                                        <div class="text-xs text-muted">Domain yang dipilih untuk
                                                            didaftarkan:</div>
                                                        <div class="font-bold text-sm text-success"
                                                            id="selected_domain_text">tokosaya.com</div>
                                                        <div class="text-xs font-semibold" style="color:var(--text);"
                                                            id="selected_domain_price_text">+ Rp 159.000 / tahun</div>
                                                    </div>
                                                    <button type="button" onclick="clearSelectedDomain()"
                                                        class="btn btn-outline btn-sm text-danger"
                                                        style="border-color:var(--danger); padding:4px 8px; font-size:11px;">
                                    ...
                                    <div id="tab_connect_domain" style="display:none;">
                                        <div class="p-3 rounded-lg"
                                            style="background:var(--surface); border:1px solid var(--border);">
                                            <label class="form-label text-xs font-semibold mb-1">Nama Custom Domain yang
                                                Sudah Anda Miliki <span style="color:var(--danger);">*</span></label>
                                            <input type="text" id="manual_custom_domain_input"
                                            ...
                                            <div
                                                style="padding:12px; border-radius:8px; background:var(--primary-soft); border:1px dashed var(--primary);">
                                                <div class="text-xs font-bold mb-1"
                                                    style="color:var(--primary); display:flex; align-items:center; gap:6px;">
                                                    <i class="fa-solid fa-server"></i> Panduan Pengaturan DNS (Arahkan ke
                                                    COOCA.ID)
                                                </div>
                                                <div class="text-xs text-muted mb-2" style="line-height:1.5;">
                                                    Arahkan DNS domain Anda di registrar domain Anda saat ini dengan salah
                                                    satu konfigurasi berikut:
                                                </div>
                                                <div class="grid-2 gap-2 text-xs"
                                                    style="font-family:monospace; background:var(--bg-secondary); padding:8px; border-radius:6px; border:1px solid var(--border);">
                                                    <div><strong>Type:</strong> CNAME<br><strong>Host:</strong> @ atau
                                                        subdomain<br><strong>Value:</strong> <code>cname.cooca.id</code>
                                                    </div>
                                                    <div><strong>Type:</strong> A Record<br><strong>Host:</strong>
                                                        @<br><strong>Value:</strong> <code>103.187.146.10</code></div>
                                                </div>
                                                <div class="mt-2 text-right">
                                                    <a href="https://wa.me/{{ $waNumber }}?text={{ $waCustomDomainMsg }}"
                                                        target="_blank" class="btn btn-outline btn-sm"
                                                        style="border-color:#25D366; color:#25D366; font-size:11px; padding:4px 10px;">
                                ...
                                <script>
                                        ...
                                        fetch(`{{ route('customer.subscriptions.check-hostinger-domain', [], false) }}?domain=${encodeURIComponent(query)}`, {
                                            ...
                                            .then(data => {
                                                ...
                                                if (data.rate_info) {
                                                    ...
                                                    if (rateBadge) {
                                                        rateBadge.innerHTML =
                                                            `<i class="fa-solid fa-coins" style="color:#F59E0B;"></i> Kurs: ${data.rate_info}`;
                                                    }
                                                }

                                                if (!data.success || !data.results || data.results.length === 0) {
                                                    list.innerHTML =
                                                        `<div class="p-3 text-xs text-muted text-center">${data.message || 'Tidak ada data domain ditemukan.'}</div>`;
                                                    return;
                                                }
                                                ...
                                                data.results.forEach(item => {
                                                    ...
                                                    if (item.is_available) {
                                                        ...
                                                        actionHtml =
                                                            `<span class="badge badge-danger" style="font-size:10px; padding:3px 8px;">Terpakai</span>`;
                                                    }
                                    ...
                                    `;
                                                ...
                                                if (data.alternatives && data.alternatives.length > 0) {
                                                    ...
                                                    altHeader.innerHTML =
                                                        '<i class="fa-solid fa-lightbulb text-warning"></i> Alternatif Rekomendasi Nama:';
                                                    ...
                                                    data.alternatives.forEach(alt => {
                                                        ...
                                                        const card = document.createElement('div');
                                                        card.className = 'p-2 rounded-lg flex items-center justify-between';
                                                        card.style.border = isSelected ? '2px solid var(--primary)' :
                                        ...
                                        `;
                                            ...
                                            .catch(err => {
                                                ...
                                                container.style.display = 'block';
                                                list.innerHTML =
                                                    `<div class="p-3 text-xs text-danger text-center">Gagal menghubungi API Hostinger: ${err.message}</div>`;
                                            });
                                    }
                                    ...
                                    function selectHostingerDomain(domain, priceIdr, priceFormatted) {
                                        ...
                                        document.getElementById('selected_domain_price_text').innerText =
                                            `+ ${priceFormatted} / tahun (Biaya Pendaftaran Domain)`;
                                ...
                                <button type="submit" class="btn btn-primary w-full"
                                    style="justify-content:center;font-size:15px;padding:14px;margin-top:4px;">
        ...
        <div style="display:flex;flex-direction:column;gap:16px;">
                ...
                <div class="card-body text-center">
                    ...
                    @else
                        <div class="product-logo-placeholder"
                            style="width:80px;height:80px;font-size:32px;border-radius:16px;margin:0 auto 12px;">
                            {{ strtoupper(substr($product->name, 0, 1)) }}
                        </div>
                    @endif
                    <div class="font-bold text-lg">{{ $product->name }}</div>
                    <div class="text-xs text-muted mb-3">v{{ $product->version ?? '5.0' }} ·
                        {{ $product->product_type_label }}</div>

                    @if ($product->demo_url)
                        <a href="{{ $product->demo_url }}" target="_blank" class="btn btn-outline btn-sm w-full mb-2"
                            style="justify-content:center;">
            ...
            @if (!$isCustom && !$activeTrial && !$activeSub)
                <div class="card" style="background:linear-gradient(135deg,#6366f1,#8B5CF6);border:none;color:#fff;">
                    <div class="card-body">
                        <div class="font-bold text-base mb-2"><i class="fa-solid fa-flask"
                                style="margin-right:6px;"></i>Coba 14 Hari Gratis</div>
                        ...
                        <a href="{{ route('customer.trials.create', ['product_id' => $product->id]) }}"
                            ...
                            style="justify-content:center;background:#fff;color:#6366f1;font-weight:700;">
            ...
            <div class="card" style="border:1px solid #25D366;">
                <div class="card-body">
                    <div class="font-bold text-sm mb-2"><i class="fa-brands fa-whatsapp"
                            style="color:#25D366;margin-right:6px;"></i>Butuh Bantuan?</div>
                    <p class="text-xs text-muted mb-3" style="line-height:1.6;">Tim kami siap membantu konsultasi, demo,
                        atau pertanyaan teknis.</p>
                    <a href="https://wa.me/{{ $waNumber }}?text={{ $waMsg }}" target="_blank"
                        class="btn btn-sm w-full"
                        style="justify-content:center;background:#25D366;color:#fff;border-color:#25D366;font-weight:600;">
            ...
            <div style="border:1px solid var(--border);border-radius:var(--radius);padding:14px;">
                <div class="text-sm font-bold mb-2"><i class="fa-solid fa-shield-halved"
                        style="color:var(--success);margin-right:6px;"></i>Kenapa COOCA.ID?</div>
