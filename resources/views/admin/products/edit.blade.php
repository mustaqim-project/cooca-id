...
@section('content')
    ...
    @if ($errors->any())
        <div
            style="background: var(--danger-soft); color: var(--danger); padding: 14px 18px; border-radius: var(--radius-sm); border: 1px solid var(--danger); margin-bottom: 24px;">
            <div style="font-weight: 700; margin-bottom: 6px;"><i class="fa-solid fa-triangle-exclamation me-1"></i> Check
                validation errors below:</div>
    ...
    <form action="{{ route('admin.products.update', $product->id) }}" method="POST" class="grid-31"
        ...
        <div class="flex-col gap-5">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="version" class="form-input" placeholder="e.g. 1.0.0"
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="requirements" class="form-textarea" rows="3"
                            placeholder="System requirements, e.g. PHP >= 8.2, MySQL >= 8.0">{{ old('requirements', $product->requirements) }}</textarea>
                    </div>
                    ...
                    <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 16px; margin-bottom: 16px;">
                        <div class="form-group">
                            ...
                            <input type="number" name="base_price" class="form-input"
                                value="{{ old('base_price', $product->base_price ?? $product->price) }}">
                        </div>
                        <div class="form-group" id="maintenance_fee_group" style="display: none;">
                            <label class="form-label">Maintenance Fee (IDR)</label>
                            <input type="number" name="maintenance_fee" class="form-input" placeholder="0"
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div id="features-container" class="flex-col gap-3">
                        ...
                        @if (is_array($currentFeatures) && count($currentFeatures) > 0)
                            @foreach ($currentFeatures as $index => $feature)
                                @php
                                    $fTitle = is_array($feature)
                                        ? $feature['title'] ?? ($feature['name'] ?? '')
                                        : $feature;
                                    $fDesc = is_array($feature) ? $feature['description'] ?? '' : '';
                                    $fIcon = is_array($feature)
                                        ? $feature['icon'] ?? 'fa-solid fa-sparkles'
                                        : 'fa-solid fa-sparkles';
                                @endphp
                                <div class="feature-item grid-31"
                                    style="align-items: start; gap: 10px; background: var(--background); padding: 12px; border-radius: 8px; border: 1px solid var(--border);">
                                    <div class="flex-col gap-2">
                                        <input type="text" name="features[{{ $index }}][title]"
                                            class="form-input" placeholder="Feature Title" value="{{ $fTitle }}"
                                            required>
                                        <input type="text" name="features[{{ $index }}][description]"
                                            class="form-input" placeholder="Feature Description"
                                            value="{{ $fDesc }}">
                                    </div>
                                    <div class="flex-col gap-2 icon-picker-wrapper" style="position: relative;">
                                        <div style="display: flex; gap: 8px; align-items: center;">
                                            <div class="icon-preview-box"
                                                style="width: 38px; height: 38px; border: 1px solid var(--border); border-radius: 6px; display: flex; align-items: center; justify-content: center; background: var(--surface); cursor: pointer;"
                                                title="Click to pick icon">
                                                <i class="{{ $fIcon }}"></i>
                                            </div>
                                            <input type="text" name="features[{{ $index }}][icon]"
                                                class="form-input icon-input"
                                                placeholder="Icon Class (e.g. fa-solid fa-warehouse)"
                                                value="{{ $fIcon }}" style="flex: 1;">
                                        </div>
                                        <div class="icon-grid-dropdown"
                                            style="display: none; position: absolute; top: 44px; right: 0; background: var(--surface); border: 1px solid var(--border); border-radius: 8px; padding: 12px; width: 240px; grid-template-columns: repeat(5, 1fr); gap: 10px; box-shadow: 0 10px 25px rgba(0,0,0,0.2); z-index: 10;">
                                        </div>
                                        <button type="button" class="btn btn-outline btn-remove-feature w-full"
                                            style="color: var(--danger); border-color: var(--danger); padding: 6px 12px; font-size: 12px;">Remove</button>
                                    </div>
                                </div>
                            @endforeach
                        @else
                            <div class="feature-item grid-31"
                                style="align-items: start; gap: 10px; background: var(--background); padding: 12px; border-radius: 8px; border: 1px solid var(--border);">
                                <div class="flex-col gap-2">
                                    <input type="text" name="features[0][title]" class="form-input"
                                        placeholder="Feature Title" required>
                                    <input type="text" name="features[0][description]" class="form-input"
                                ...
                                <div class="flex-col gap-2 icon-picker-wrapper" style="position: relative;">
                                    <div style="display: flex; gap: 8px; align-items: center;">
                                        ...
                                        <input type="text" name="features[0][icon]" class="form-input icon-input"
                                            placeholder="Icon Class" value="fa-solid fa-sparkles" style="flex: 1;">
                                    </div>
                                    <div class="icon-grid-dropdown"
                                        style="display: none; position: absolute; top: 44px; right: 0; background: var(--surface); border: 1px solid var(--border); border-radius: 8px; padding: 12px; width: 240px; grid-template-columns: repeat(5, 1fr); gap: 10px; box-shadow: 0 10px 25px rgba(0,0,0,0.2); z-index: 10;">
                                    </div>
                    ...
                    <button type="button" id="btn-add-feature" class="btn btn-outline mt-3 w-full border-dashed"
        ...
        <div class="flex-col gap-5">
                ...
                <div class="card-body">
                    <div class="form-group">
                        ...
                        <select name="is_active" class="form-select">
                            <option value="1" {{ old('is_active', $product->is_active) ? 'selected' : '' }}>Active
                                (Visible on Catalog & Landing)</option>
                    ...
                    <div class="form-group mt-3">
                        ...
                        <select name="is_featured" class="form-select">
                            <option value="0" {{ !old('is_featured', $product->is_featured) ? 'selected' : '' }}>
                                Standard Product</option>
                            <option value="1" {{ old('is_featured', $product->is_featured) ? 'selected' : '' }}>
                                Featured / Popular Product</option>
                        </select>
                    </div>
                ...
                <div class="card-body">
                    ...
                    <div class="form-group">
                        ...
                        <input type="file" name="thumbnail" class="form-input" accept="image/*"
                            style="padding: 10px;">
                        <small style="color: var(--text-muted); font-size: 12px; margin-top: 4px; display: block;">Upload a
                            new image to replace the current one (Max 2MB).</small>
                    </div>
                </div>
            </div>
            ...
            <div class="card">
                <div class="card-header">
                    <div class="card-title">Manage Pricing & Plans</div>
                </div>
                <div class="card-body">
                    <p class="text-sm text-muted mb-4">Set up recurring subscription tiers, billing cycles, trial
                        durations, and discount rates for this product.</p>
                    <div style="max-width: 320px;">
                        <a href="{{ route('admin.products.plans.index', $product->id) }}"
                            class="btn btn-outline w-full"><i class="fa-solid fa-tags mr-1"></i> Manage Pricing Plans
                            ({{ $plans->count() }})</a>
                    </div>
                </div>
            </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const popularIcons = [
                ...
                'fa-solid fa-sparkles', 'fa-solid fa-bolt', 'fa-solid fa-shield', 'fa-solid fa-star',
                'fa-solid fa-check',
                'fa-solid fa-rocket', 'fa-solid fa-mobile-screen', 'fa-solid fa-users', 'fa-solid fa-headset',
                'fa-solid fa-credit-card', 'fa-solid fa-globe', 'fa-solid fa-cloud', 'fa-solid fa-lock',
                'fa-solid fa-server',
                'fa-solid fa-code'
            ];
            ...
            function populateGrid(dropdown) {
                ...
                popularIcons.forEach(icon => {
                    html +=
                        `<div class="icon-option" data-icon="${icon}" style="display:flex; align-items:center; justify-content:center; padding:8px; border-radius:4px; cursor:pointer; background:rgba(0,0,0,0.05);" onmouseover="this.style.background='rgba(79,70,229,0.1)'" onmouseout="this.style.background='rgba(0,0,0,0.05)'"><i class="${icon}"></i></div>`;
                });
