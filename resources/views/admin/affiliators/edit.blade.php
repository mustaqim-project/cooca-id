...
@section('subtitle', 'Update affiliator information')
...
<div class="space-y-6 max-w-4xl mx-auto">
    ...
    <div class="corporate-card">
        ...
        <form action="{{ route('admin.affiliators.update', $affiliator->id) }}" method="POST" class="p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $affiliator->name) }}" required 
                ...
                <div>
                    ...
                    <input type="email" name="email" id="email" value="{{ old('email', $affiliator->email) }}" required 
                        class="block w-full py-2 px-3 border border-surface-300 dark:border-surface-600 bg-white dark:bg-surface-800 rounded-md shadow-sm focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-sm text-surface-900 dark:text-white">
                    @error('email')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="md:col-span-2">
                    <label class="flex items-center space-x-3">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $affiliator->is_active) ? 'checked' : '' }}
                            class="h-4 w-4 text-primary-600 focus:ring-primary-500 border-surface-300 rounded dark:border-surface-600 dark:bg-surface-700 dark:checked:bg-primary-500">
                        <span class="text-sm font-medium text-surface-700 dark:text-surface-300">Active Account</span>
                    </label>
                    @error('is_active')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
