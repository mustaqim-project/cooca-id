...
@section('content')
    <div class="d-flex flex-column gap-4" style="max-width: 800px; margin: 0 auto;">
        ...
        <div class="d-flex align-items-center gap-3">
            ...
            <div>
                <h2 class="mb-1 fw-bold">Edit Testimonial</h2>
                <p class="text-secondary mb-0">Update client review or rating details.</p>
            </div>
        </div>
        ...
        <div class="card border-0 shadow-sm rounded-4 glass p-4 p-md-5">
            <form action="{{ route('admin.testimonials.update', $testimonial->id ?? 1) }}" method="POST"
                ...
                <div class="d-flex flex-column align-items-center justify-content-center text-center p-4 border rounded-4 border-dashed bg-light-subtle position-relative overflow-hidden hover-lift cursor-pointer"
                    ...
                    <div class="mb-2">
                        <img src="{{ $testimonial->avatar ?? 'https://ui-avatars.com/api/?name=' . urlencode($testimonial->name ?? 'Sarah Jenkins') . '&background=6366f1&color=fff' }}"
                            class="rounded-circle border shadow-sm" width="80" height="80" alt="Avatar">
                    </div>
                    <h6 class="fw-medium mb-1">Change Avatar</h6>
                    <p class="fs-7 text-secondary mb-0">Click or drag image to replace (JPG, PNG)</p>
                </div>
                    ...
                    <div class="col-12 col-md-6">
                        <div class="form-floating">
                            <input type="text" class="form-control rounded-3 shadow-none border bg-transparent"
                                ...
                                value="{{ $testimonial->name ?? 'Sarah Jenkins' }}" required>
                    ...
                    <div class="col-12 col-md-6">
                        <div class="form-floating">
                            <input type="text" class="form-control rounded-3 shadow-none border bg-transparent"
                                ...
                                value="{{ $testimonial->role ?? 'VP of Operations' }}">
                    ...
                    <div class="col-12 col-md-6">
                        <div class="form-floating">
                            <input type="text" class="form-control rounded-3 shadow-none border bg-transparent"
                                ...
                                value="{{ $testimonial->company ?? 'Apex Global' }}">
                    ...
                    <div class="col-12 col-md-6">
                        <div class="form-floating">
                            <select class="form-select rounded-3 shadow-none border bg-transparent" id="rating"
                                ...
                                <option value="5" {{ ($testimonial->rating ?? 5) == 5 ? 'selected' : '' }}>5 Stars
                                </option>
                                <option value="4" {{ ($testimonial->rating ?? 5) == 4 ? 'selected' : '' }}>4 Stars
                                </option>
                                <option value="3" {{ ($testimonial->rating ?? 5) == 3 ? 'selected' : '' }}>3 Stars
                                </option>
                                <option value="2" {{ ($testimonial->rating ?? 5) == 2 ? 'selected' : '' }}>2 Stars
                                </option>
                                <option value="1" {{ ($testimonial->rating ?? 5) == 1 ? 'selected' : '' }}>1 Star
                                </option>
                            </select>
                    ...
                    <div class="col-12 col-md-6">
                        <div class="form-floating">
                            <select class="form-select rounded-3 shadow-none border bg-transparent" id="is_active"
                                ...
                                <option value="1" {{ $testimonial->is_active ?? true ? 'selected' : '' }}>Active
                                    (Featured on Landing Page)</option>
                                <option value="0" {{ !($testimonial->is_active ?? true) ? 'selected' : '' }}>Hidden
                                </option>
                            </select>
                    ...
                    <div class="col-12">
                        <div class="form-floating">
                            <textarea class="form-control rounded-3 shadow-none border bg-transparent" id="quote" name="quote"
                                placeholder="Quote / Feedback" style="height: 120px" required>{{ $testimonial->quote ?? 'Cooca-ID streamlined our inventory and ERP workflows effortlessly.' }}</textarea>
                ...
                <div class="d-flex justify-content-end gap-2">
                    ...
                    <button type="submit" class="btn btn-primary rounded-pill px-4 shadow-sm hover-lift">
                        <i class="bi bi-check2 me-2"></i> Update Testimonial
                    </button>
                </div>
            </form>
        </div>
