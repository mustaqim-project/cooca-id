    ...
    <div class="d-flex flex-column gap-4" style="max-width: 1000px; margin: 0 auto;">
        ...
        <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3">
            <div class="d-flex align-items-center gap-3">
                ...
                <div>
                    <h2 class="mb-1 fw-bold text-capitalize">Edit Blog Post</h2>
                    <p class="text-secondary mb-0">Update content and publish settings for
                        "{{ $post->title ?? 'Top 5 ERP Trends in 2026' }}".</p>
                </div>
            </div>
            <div class="d-flex gap-2">
                <a href="{{ route('admin.blog.show', $post->id ?? 1) }}"
        ...
        <div class="card border-0 shadow-sm rounded-4 glass p-4 p-md-5">
            <form action="{{ route('admin.blog.update', $post->id ?? 1) }}" method="POST" enctype="multipart/form-data"
                ...
                <div class="row g-4">
                    <div class="col-12 col-md-8">
                        <div class="form-floating">
                            <input type="text"
                                ...
                                id="title" name="title"
                                value="{{ old('title', $post->title ?? 'Top 5 ERP Trends in 2026') }}"
                                placeholder="Article Title" onkeyup="generateSlug()" required>
                    ...
                    <div class="col-12 col-md-4">
                        <div class="form-floating">
                            <input type="text"
                                ...
                                value="{{ old('category', $post->category ?? 'Technology') }}" placeholder="Category"
                                required>
                    ...
                    <div class="col-12 col-md-8">
                        <div class="form-floating">
                            <input type="text"
                                ...
                                id="slug" name="slug"
                                value="{{ old('slug', $post->slug ?? 'top-5-erp-trends-in-2026') }}" placeholder="url-slug"
                                required>
                    ...
                    <div class="col-12 col-md-4">
                        <div class="form-floating">
                            <select
                                ...
                                @php $isPublished = old('is_published', ($post->is_published ?? true) ? '1' : '0'); @endphp
                    ...
                    <div class="col-12">
                        ...
                        <div
                            ...
                            <div id="upload-prompt" class="{{ isset($post->image) ? 'd-none' : '' }}">
                                <i class="bi bi-image fs-1 text-secondary mb-2"></i>
                                <h6 class="fw-medium mb-1">Click to upload new cover image</h6>
                                <p class="fs-7 text-secondary mb-0">Leave blank to keep existing image</p>
                            </div>
                            <img id="image-preview"
                                src="{{ $post->image ?? 'https://images.unsplash.com/photo-1460925895917-afdab827c52f?auto=format&fit=crop&w=800&q=80' }}"
                                alt="Preview"
                                class="img-fluid rounded-3 {{ !isset($post->image) && !old('image') ? '' : 'd-none' }}"
                    ...
                    <div class="col-12">
                        ...
                        <div class="border rounded-3 p-2 bg-light">
                            ...
                            <textarea class="form-control border-0 shadow-none bg-transparent @error('content') is-invalid @enderror"
                                id="content" name="content" placeholder="Write your article content here..."
                                style="min-height: 400px; resize: vertical;" required>{{ old('content', $post->content ?? '<h2>Introduction</h2><p>Enterprise Resource Planning (ERP) is evolving faster than ever before...</p>') }}</textarea>
                        </div>
                        @error('content')
                ...
                <div class="d-flex justify-content-end gap-2">
                    ...
                    <button type="submit" class="btn btn-primary rounded-pill px-4 shadow-sm hover-lift">
                        <i class="bi bi-check2 me-2"></i> Update Post
                    </button>
                </div>
            </form>
        </div>
