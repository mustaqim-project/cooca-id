...
@section('content')
    <div class="d-flex flex-column gap-4">
        ...
        <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3">
            <div>
                <h2 class="mb-1 fw-bold text-capitalize">Product Categories</h2>
                <p class="text-secondary mb-0">Manage and view product categories data.</p>
            </div>
                ...
                <table class="table table-hover align-middle mb-0" style="color: var(--color-text-primary);">
                    ...
                    <tbody class="border-top-0">
                        @forelse($categories as $category)
                            <tr>
                                <td class="py-3 px-4 text-secondary fs-7">#{{ $category->id }}</td>
                                <td class="py-3 px-3">
                                    <div class="fw-medium">{{ $category->name }}</div>
                                    @if ($category->description)
                                        <div class="text-secondary fs-7">{{ Str::limit($category->description, 50) }}</div>
                                    @endif
                                </td>
                                <td class="py-3 px-3">
                                ...
                                <td class="py-3 px-3 text-secondary fs-7">{{ $category->created_at->format('d M Y') }}</td>
                                    ...
                                    <div class="dropdown">
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-end shadow-lg border-0 rounded-3 glass">
                                            <li><a class="dropdown-item py-2"
                                                    href="{{ route('admin.product-categories.show', $category->id) }}"><i
                                                        class="bi bi-eye me-2 text-primary"></i> View Details</a></li>
                                            <li><a class="dropdown-item py-2"
                                                    href="{{ route('admin.product-categories.edit', $category->id) }}"><i
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="py-5 text-center text-secondary">
                                    ...
                                    <p class="fs-7">Create your first product category.</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
