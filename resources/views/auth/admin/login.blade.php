    ...
    <div class="auth-layout">
        <div class="auth-left auth-panel">
            ...
            <div class="auth-left-content">
                ...
                <h2>{{ __('Admin') }} <span class="text-gradient">{{ __('Portal') }}</span></h2>
                <p style="font-size:.95rem;color:var(--text-muted);">{{ __('Secure access to your admin dashboard.') }}</p>
            </div>
        </div>
        <div class="auth-right auth-panel">
            <div class="auth-form-panel">
                <div class="form-title" style="font-size:1.7rem;font-weight:800;margin-bottom:4px;">{{ __('Admin Login') }}
                </div>
                <p class="mb-4" style="font-size:.9rem;">{{ __('Sign in to manage your admin account.') }}</p>
                ...
                <form action="{{ route('admin.login.submit') }}" method="POST">
                    ...
                    <button type="submit" class="btn btn-primary btn-block btn-lg">{{ __('Log In') }} <i
