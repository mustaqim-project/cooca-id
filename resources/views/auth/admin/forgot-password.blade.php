    ...
    <div class="auth-layout">
        ...
        <div class="auth-right auth-panel">
            <div class="auth-form-panel">
                <div class="form-title" style="font-size:1.7rem;font-weight:800;margin-bottom:4px;">
                    {{ __('Reset Password') }}</div>
                <p class="mb-4" style="font-size:.9rem;">{{ __('Enter your email to receive a password reset link.') }}
                </p>
                ...
                <form action="{{ route('admin.password.request') }}" method="POST">
                    ...
                    <button type="submit" class="btn btn-primary btn-block btn-lg">{{ __('Send Reset Link') }} <i
                            class="bi bi-send"></i></button>
                </form>
                <p class="text-center mt-4"><a href="{{ route('admin.login') }}"><i class="bi bi-arrow-left"></i>
                        {{ __('Back to Login') }}</a></p>
            </div>
        </div>
    </div>
@endsection
